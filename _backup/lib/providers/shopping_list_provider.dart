	...
	Future<void> generateShoppingListFromMealPlans(List<MealPlan> mealPlans) async {
		try {
			...
			for (var entry in ingredientCounts.entries) {
				final itemName = entry.value > 1 ? '${entry.key} (${entry.value}x)' : entry.key;
				debugPrint('Adding item: $itemName');
	...
	String _categorizeIngredient(String ingredient) {
		...
		final categoryMaps = {
			'Meat & Seafood': [
				...
				'turkey', 'lamb', 'crab', 'lobster', 'bacon', 'ham', 'sausage'
			],
			'Dairy': [
			...
			'Produce': [
				'apple', 'banana', 'berry', 'orange', 'tomato', 'onion', 'carrot',
				'lettuce', 'spinach', 'broccoli', 'potato', 'bell pepper', 'cucumber',
				'avocado', 'lemon', 'lime', 'garlic', 'ginger', 'mushroom'
			],
			'Grains': [
				'bread', 'pasta', 'rice', 'cereal', 'flour', 'oats', 'quinoa',
				'barley', 'wheat', 'bagel', 'tortilla', 'crackers'
			],
			'Pantry': [
				'oil', 'vinegar', 'salt', 'pepper', 'sugar', 'spices', 'herbs',
				'sauce', 'dressing', 'honey', 'vanilla', 'baking powder', 'soy sauce'
			],
			'Frozen': [
